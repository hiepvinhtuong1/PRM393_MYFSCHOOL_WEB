-- =============================================================================
-- V9: Xóa sạch dữ liệu cũ và reseed lại toàn bộ
-- (chạy khi V8 bị skip do DB đã có dữ liệu từ trước)
-- time_slots và score_components giữ nguyên (seeded từ V2)
-- =============================================================================
...
-- =============================================================================
-- Reseed (giống V8 nhưng không có guard)
-- =============================================================================
...
FOR i IN 1..60 LOOP
  stu_full_name :=
    last_names[((i-1)%10)+1]||' '||first_names[((i-1)%20)+1];
...
-- ================================================================
-- TIẾT HỌC + ĐIỂM DANH
-- TKB: sub_wc 1-3→Thứ2, 4-6→Thứ3, 7-9→Thứ4, 10→Thứ5
--       slot: (sub_wc-1)%3 → 1,3,5
-- HK I: 15 tiết/cs (2025-09-01 là Thứ 2)
-- HK II: 10 tiết/cs (2026-01-05 là Thứ 2)
-- ================================================================
FOR i IN 1..60 LOOP
  sub_wc    := ((i-1)%10)+1;
  dow       := (sub_wc-1)/3;
  ...
  FOR k IN 0..14 LOOP
    ...
    INSERT INTO lessons (
      ...
      start_slot_id, end_slot_id, room_id, status, has_materials
    ) VALUES (
    ...
    FOR s IN 1..10 LOOP
      att_status := CASE
        ...
        ELSE 'present'
      END;
      INSERT INTO attendance_records (student_id, lesson_id, status, recorded_by, recorded_at)
      VALUES (
        stu_ids[cl_offset+s], lid, att_status,
        t_user_ids[((i-1)%10)+1],
...
FOR i IN 1..60 LOOP
  ...
  FOR s IN 1..10 LOOP
    FOR k IN 1..5 LOOP
      ...
      VALUES (
        stu_ids[cl_offset+s], cs_hk1[i], k,
...
VALUES ('Lịch nghỉ Tết Nguyên Đán 2026',
  'Nhà trường thông báo lịch nghỉ Tết Nguyên Đán từ ngày 26/01/2026 đến 03/02/2026. Học sinh quay lại trường vào ngày 04/02/2026.',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Hội thao truyền thống 2026',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Họp phụ huynh tổng kết HK I năm học 2025-2026',
  'Nhà trường tổ chức họp phụ huynh tổng kết học kỳ I vào ngày 22/12/2025. Đề nghị phụ huynh sắp xếp thời gian tham dự.',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Tuần lễ hướng nghiệp 2026 — dành cho lớp 12',
  'Chương trình Tuần lễ hướng nghiệp dành cho học sinh lớp 12 diễn ra từ 09-13/03/2026. Đăng ký tại văn phòng trước 28/02/2026.',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Thi thử THPT Quốc gia đợt 1 — lớp 12',
  'Nhà trường tổ chức kỳ thi thử THPT Quốc gia đợt 1 vào ngày 25-26/02/2026. Lịch chi tiết sẽ được thông báo đầu tháng 2.',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Khai giảng năm học 2025-2026',
  'Lễ Khai giảng năm học 2025-2026 được tổ chức 07:30 ngày 05/09/2025. Học sinh mặc đồng phục, có mặt trước 07:00.',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('[10A1] Nhắc nhở đồng phục HK II',
  'Học sinh lớp 10A1 lưu ý mặc đúng đồng phục quy định trong HK II. Tuần sau ban giám thị sẽ kiểm tra toàn trường.',
...
INSERT INTO notification_recipients (notification_id,user_id)
SELECT DISTINCT n_id,p.user_id FROM parent_students ps
JOIN students st ON ps.student_id=st.id AND st.classroom_id=cl_ids[1]
JOIN parents p ON ps.parent_id=p.id WHERE p.user_id IS NOT NULL;

INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('[11B1] Lịch tiết học bù môn Toán',
  'Lớp 11B1 có tiết bù môn Toán vào thứ Bảy 24/01/2026 (tiết 1-2, phòng A1.03).',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('[12C1] Tổng kết HK I — xếp hạng 1 khối 12',
  '9/10 học sinh đạt học lực Giỏi, 1 em Khá. Lớp 12C1 xếp hạng 1 toàn khối. Tiếp tục phát huy trong HK II.',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Cảnh báo: Số buổi nghỉ vượt ngưỡng quy định',
  'Học sinh đã nghỉ học không phép 4 buổi trong học kỳ này. Đề nghị phụ huynh phối hợp với nhà trường.',
...
INSERT INTO notification_recipients (notification_id,user_id)
SELECT DISTINCT n_id,p.user_id FROM parent_students ps
JOIN parents p ON ps.parent_id=p.id WHERE ps.student_id=stu_ids[3] AND p.user_id IS NOT NULL;

INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Vắng học không phép ngày 05/03/2026',
  'Học sinh vắng mặt không có lý do ngày 05/03/2026. Đề nghị phụ huynh liên hệ GVCN để giải trình.',
...
INSERT INTO notification_recipients (notification_id,user_id)
SELECT DISTINCT n_id,p.user_id FROM parent_students ps
JOIN parents p ON ps.parent_id=p.id WHERE ps.student_id=stu_ids[18] AND p.user_id IS NOT NULL;

INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Kết quả thi HK I 2025-2026 đã cập nhật',
  'Điểm thi học kỳ I đã được nhập đầy đủ. Học sinh và phụ huynh xem bảng điểm trên ứng dụng MyFPTSchool.',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Điểm thường xuyên tháng 10/2025 đã cập nhật',
  'Giáo viên đã nhập điểm TX tháng 10/2025. Học sinh kiểm tra và phản hồi sai sót trước 05/11/2025.',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Điểm giữa kỳ II đã được nhập',
  'Điểm DGKK HK II đã nhập xong. Điểm cuối kỳ sẽ được nhập sau kỳ thi tháng 5.',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Tài liệu ôn thi HK I đã đăng tải',
  'Đề cương và tài liệu ôn tập HK I đã có trên hệ thống. Học sinh vào mục Tài liệu trên ứng dụng để tải về.',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Lịch kiểm tra định kỳ tháng 3/2026',
  'Toán (02/03), Văn (04/03), Anh (06/03), Lý (09/03), Hóa (11/03). Học sinh chú ý ôn tập.',
...
INSERT INTO notifications (title,body,category,target_type,target_id,created_by,created_at)
VALUES ('Hội thi Olympic Toán và Tin học cấp trường 2026',
  'Tổ chức ngày 14-15/04/2026. Đăng ký qua GVCN trước 31/03/2026. Giải thưởng hấp dẫn!',
  'study','all',NULL,admin_uid,'2026-03-20')
RETURNING id INTO n_id;
INSERT INTO notification_recipients (notification_id,user_id)
SELECT n_id,u.id FROM users u WHERE u.role IN ('STUDENT','PARENT');

RAISE NOTICE 'V9: Reseed hoàn thành.';
END $seed$;
